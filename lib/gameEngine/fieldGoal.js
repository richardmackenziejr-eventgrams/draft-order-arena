...
function stopDirection(state, memberId, clientElapsedMs) {
  ...
  // this kick, period — no partial credit for being close outside it.
  const hadPower = Math.abs(player.powerPos - 0.5) <= powerSweetHalfFor(kick.distance);
...
module.exports = {
  ...
  KICKS_PER_PLAYER, POWER_PERIOD_MS, DIRECTION_PERIOD_MS,
  trianglePosition,
