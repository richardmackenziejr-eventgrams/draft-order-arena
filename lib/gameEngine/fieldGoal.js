...
const description = 'A classic two-click retro kicker: stop the power meter in the sweet spot, then stop the direction meter to split the uprights — beating the wind along the way. 10 kicks per player on a rising/falling difficulty ladder: start at 30-39 yards, move up a tier on a make, drop back down on a miss.';
...
// from under a kick already in progress or already locked) and starts its
// power meter. Called each time the player's view is fetched, but the clock
// only arms once, while that kick is still unresolved and the power phase
...
function presentCurrentKick(state, memberId) {
  ...
  if (player.attempts[idx] == null) {
    if (player.kicks[idx] == null) {
...
module.exports = {
  ...
  trianglePosition, powerSweetHalfFor, rollKick, nextTierFor,
  initInstance, getOrCreatePlayer, presentCurrentKick, currentKickView,
  stopPower, stopDirection, resyncClock, advanceToNext, isComplete, computeResults,
};
