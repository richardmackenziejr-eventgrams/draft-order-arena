...
// Locks in the power meter's position from real elapsed time — idempotent,
// so a duplicate/retried request doesn't re-roll it. Immediately arms the
// direction meter's clock too, since locking power is what starts it.
function stopPower(state, memberId) {
  const player = getOrCreatePlayer(state, memberId);
  ...
  const elapsedMs = Math.max(0, Date.now() - player.powerStartedAt);
...
function stopDirection(state, memberId) {
  const player = getOrCreatePlayer(state, memberId);
  if (player.completed) return { error: 'already-completed' };
  const idx = player.currentIndex;
  ...
  const elapsedMs = Math.max(0, Date.now() - player.directionStartedAt);
