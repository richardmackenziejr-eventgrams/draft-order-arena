// 40 Yard Dash — the commissioner assigns each member an "odds weight"
...
//            (best to worst — pick #1 first, like the winner of an actual race)
//            via sockets/liveRooms.js.
const crypto = require('crypto');

const id = 'lottery';
const name = '40 Yard Dash';
const description = 'A single weighted draw decides the whole order at once — set odds from last year\'s standings, or make it completely random. Watch it play out as a 40-yard dash, just like the NFL Combine.';
...
function initInstance(config, members) {
  const cfg = {
    odds: { ...defaultConfig(members).odds, ...(config.odds || {}) },
    // Purely informational — the odds values already encode whether this was
    // randomized or standings-based; this just lets the UI say which it was.
...
module.exports = { id, name, description, category, supportedModes, defaultConfig, runDraw, initInstance, toResults };
