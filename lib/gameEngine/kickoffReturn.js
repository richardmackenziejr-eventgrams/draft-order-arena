...
// runner with arrow keys to dodge defenders on a horizontally-scrolling
// field (classic Tecmo Bowl style — no juke/spin moves; the only way past
// a defender is reading their dive and changing direction before it lands),
// entirely simulated in the browser. This engine never runs that simulation
// itself — it only decides how hard the NEXT return should be (how many
// defenders, how fast) and scores whatever outcome the client reports back,
// the same "server decides the setup and trusts the client's report" split
// every other async game here already uses (e.g. Field Goal Kick trusting
// the client's own elapsed-time reading).
//
// 5 returns per player, each starting at the player's own goal line (100
// yards from the opposing goal line — an authentic Tecmo-style catch right
// at the goal, not the NFL's usual touchback spot). Every return scores 0.1
// points per yard gained plus a 7-point bonus for reaching the end zone (a
// touchdown is always exactly 100 yards, so a max return is worth 10 + 7 =
// 17; a max session is 85). A per-player difficulty LEVEL — not a named
// tier like Field Goal's short/mid/long ladder, since this needs to climb
...
const description = 'Real-time and keyboard-controlled: catch the kick at your own goal line and dodge your way upfield with the arrow keys — no juke or spin, just like the original. 5 returns per player — the better you do, the tougher the coverage gets on your next one.';
const category = 'arcade';
const supportedModes = ['async'];

const RETURNS_PER_PLAYER = 5;
const FIELD_YARDS = 100; // own goal line to the opposing goal line
