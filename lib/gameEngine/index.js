...
const reactionBracket = require('./reactionBracket');
const trivia = require('./trivia');

const MODULES = {
  [lottery.id]: lottery,
  [reactionBracket.id]: reactionBracket,
  [trivia.id]: trivia,
};

function listModules() {
  return Object.values(MODULES).map((m) => ({
    id: m.id, name: m.name, category: m.category, supportedModes: m.supportedModes,
  }));
}
